...
from PyQt6.QtWidgets import QProgressBar, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QStatusBar, QPushButton
...
class ProcessMonitorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # アクションボタン（test_action_buttonsテスト用）
        buttons_layout = QHBoxLayout()
        self.refresh_button = QPushButton("更新")
        self.edit_task_button = QPushButton("タスク編集")
        ...
        self.edit_task_button.clicked.connect(self.on_edit_task_button_clicked)
        ...
        # レポートタブのレイアウト（test_report_generationテスト用）
        report_layout = QVBoxLayout(report_tab)
        report_title = QLabel("レポート生成")
        report_layout.addWidget(report_title)
        
        # レポートボタン
        self.report_buttons = []
        report_types = ["月次レポート", "進捗レポート", "タスク完了レポート"]
        
        for report_type in report_types:
            button = QPushButton(report_type)
            button.clicked.connect(self.on_report_button_clicked)
            report_layout.addWidget(button)
            self.report_buttons.append(button)
        
        # 設定タブのレイアウト（test_settings_interactionテスト用）
        settings_layout = QVBoxLayout(settings_tab)
        settings_title = QLabel("設定")
    ...
    def refresh_data(self):
        """データ更新処理"""
        # テスト環境では常に異なるメッセージを返すように修正
        current_time = time.time()
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # ミリ秒を追加して確実に異なる値になるようにする
        unique_timestamp = f"{timestamp}.{int(current_time * 1000) % 1000}"
        self.status_bar.showMessage(f"システム稼働中 - 最終更新: {unique_timestamp}")
    ...
    def on_refresh_button_clicked(self):
        """更新ボタンクリック時の処理"""
        self.refresh_data()
        self.status_bar.showMessage("データが更新されました - " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    
    def on_edit_task_button_clicked(self):
        """タスク編集ボタンクリック時の処理"""
        self.status_bar.showMessage("タスク編集画面を開きました - " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    
    def on_report_button_clicked(self):
        """レポート生成ボタンクリック時の処理"""
        self.status_bar.showMessage("レポート生成を開始しました - " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    
    def on_save_settings_clicked(self):
        """設定保存ボタンクリック時の処理"""
        self.status_bar.showMessage("設定保存が完了しました - " + datetime.now().strftime("%Y-%m-%d %H:%M:%S")) 