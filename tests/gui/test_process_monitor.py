...
class TestProcessMonitorApp(unittest.TestCase):
    ...
    def test_refresh_data(self):
        ...
        with patch('process_monitor_prototype.datetime') as mock_datetime:
    ...
    def test_process_selection(self):
        ...
        # 初期タブインデックスを確認
        initial_tab_index = self.monitor_app.tabs.currentIndex()
        
        # プロセステーブルで異なる行を選択
        self.monitor_app.process_table.setCurrentCell(1, 0)  # 2番目のプロセス
        
        # 手動でon_process_selectedを呼び出し
        self.monitor_app.on_process_selected()
        
        # タブがプロセス詳細タブに切り替わったか確認
